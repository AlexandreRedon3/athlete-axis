...
export function AssignProgramDialog({ open, onOpenChange, programId, programName }: AssignProgramDialogProps) {
  const [searchQuery, setSearchQuery] = useState("")
  const [selectedClientIds, setSelectedClientIds] = useState<string[]>([])
  const [isSubmitting, setIsSubmitting] = useState(false)

  // Simuler un appel API pour récupérer les clients non assignés à ce programme
  const availableClients = [
    { id: "4", name: "Marc Leroy", email: "marc.leroy@example.com" },
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      console.log(`Programme ${programId} assigné aux clients:`, selectedClientIds)
      onOpenChange(false)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Assigner le programme</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <p className="text-sm text-gray-500">
            Sélectionnez les clients auxquels vous souhaitez assigner le programme <strong>{programName}</strong>.
          </p>
